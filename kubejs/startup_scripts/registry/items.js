StartupEvents.registry('item', e => {

    // Research: //
    // 1x
    e.create('basic_tech_card').texture('kubejs:item/tech_cards/basic_tech_card')
    e.create('automation_tech_card').texture('kubejs:item/tech_cards/automation_tech_card')
    e.create('logistic_tech_card').texture('kubejs:item/tech_cards/logistic_tech_card')
    e.create('agriculture_tech_card').texture('kubejs:item/tech_cards/agriculture_tech_card')
    e.create('chemical_tech_card').texture('kubejs:item/tech_cards/chemical_tech_card')
    e.create('production_tech_card').texture('kubejs:item/tech_cards/production_tech_card')
    e.create('utility_tech_card').texture('kubejs:item/tech_cards/utility_tech_card')
    e.create('quantum_tech_card').texture('kubejs:item/tech_cards/quantum_tech_card')

    // 8x
    e.create('basic_tech_card_8x')
    e.create('automation_tech_card_8x')
    e.create('logistic_tech_card_8x')
    e.create('agriculture_tech_card_8x')
    e.create('chemical_tech_card_8x')
    e.create('production_tech_card_8x')
    e.create('utility_tech_card_8x')
    e.create('quantum_tech_card_8x')

    // 64x
    e.create('basic_tech_card_64x')
    e.create('automation_tech_card_64x')
    e.create('logistic_tech_card_64x')
    e.create('agriculture_tech_card_64x')
    e.create('chemical_tech_card_64x')
    e.create('production_tech_card_64x')
    e.create('utility_tech_card_64x')
    e.create('quantum_tech_card_64x')

    //misc
    e.create('blank_tech_card')

    // It's over.
    e.create('pyrrhic_victory')
})